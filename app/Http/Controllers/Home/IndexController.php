...
class IndexController extends CommonController
{
    ...
    public function cate( $cate_id )
    {
        ...
        //当前分类的子分类
        $submenu = Category::where( 'cate_pid',$cate_id )->get()->toarray();
    ...
    {
        $input = Input::get('s');
        $data = Article::leftJoin('category as ca','article.cate_id','=','ca.id')
            ->select('article.id','art_title','art_tag','art_description','art_thumb','art_content','art_time','art_editor','art_view','cate_id','ca.cate_name')
            ->where('art_title', 'like', '%' . urldecode($input) . '%')->orderBy( 'art_time','desc')->paginate(4);
        //print_r($data);
        return view( 'home.serch',compact( 'input','data' ));
    }
}
